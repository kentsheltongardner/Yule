...
export default class Game {
    ...
    act(moveDir: number) {
        ...
        if (!this.inBounds(movePoint.x, movePoint.y)) return
    ...
    attemptMove(movePoints: Point[], moveDir: number) {
        const moveVec2  = Dir.ToVec2[moveDir]
    ...
    canMove(x: number, y: number, dx: number, dy: number): boolean {
        ...
        if (cell.type === Cell.Wall)    return false
        if (cell.type === Cell.Empty)   return true

        cell.flag = true
        if (!this.canMove(x + dx, y + dy, dx, dy)) return false
        ...
        for (const dir of Dir.Dirs) {
            ...
            if (conn) {
                ...
                if (!this.canMove(connX, connY, dx, dy)) return false
            }
        }
    ...
    renderCells() {
        ...
        for (let i = 0; i < w; i++) {
            for (let j = 0; j < h; j++) {
                const cell = grid[i][j]
                if (cell.type === Cell.Empty) continue
                if (cell.type === Cell.Tunnel) continue
    ...
    renderPresents() {
        ...
        for (let i = 0; i < w; i++) {
            for (let j = 0; j < h; j++) {
                const cell = grid[i][j]
                if (cell.type !== Cell.Present) continue
    ...
    renderLasers() {
        ...
        const lasers = (color: string, thickness: number, radius: number) => {
    ...
    renderBoxes() {
        ...
        const interiorRender = (fillStyle: string, offset: number) => {
